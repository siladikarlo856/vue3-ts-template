var oidc = (() => {
  ...
  var r =
    ...
    function (r2) {
      ...
      for (
        var n2, o2, a = 0, i = 0, c = "";
        (o2 = t2.charAt(i++));
        ~o2 && ((n2 = a % 4 ? 64 * n2 + o2 : o2), a++ % 4)
          ? (c += String.fromCharCode(255 & (n2 >> ((-2 * a) & 6))))
          : 0
      )
        o2 =
  ...
  var ErrorResponse = class extends Error {
    constructor(args, form) {
      ...
      this.error_description =
        (_a = args.error_description) != null ? _a : null;
  ...
  var CheckSessionIFrame = class {
    constructor(_callback, _client_id, url, _intervalInSeconds, _stopOnError) {
      ...
      this._message = (e2) => {
        if (
          e2.origin === this._frame_origin &&
          e2.source === this._frame.contentWindow
        ) {
          if (e2.data === "error") {
            ...
            this._logger.debug(
              e2.data + " message from check session op iframe"
            );
          }
        }
      };
  ...
  var JsonService = class {
    ...
    async postForm(url, { body, basicAuth, timeoutInSeconds }) {
    ...
    async revoke(args) {
      ...
      logger2.debug(
        `got revocation endpoint, revoking ${
          (_a = args.token_type_hint) != null ? _a : "default token type"
        }`
      );
  ...
  var ResponseValidator = class {
    ...
    async _processClaims(response, skipUserInfo = false, validateSub = true) {
      ...
      response.profile = this._mergeClaims(
        response.profile,
        this._filterProtocolClaims(claims)
      );
      logger2.debug(
  ...
  var OidcClient = class {
    ...
    async useRefreshToken({ state, timeoutInSeconds, customRequestArgs = {} }) {
    ...
    async navigate(params) {
      ...
      const { url, keepOpen } = await new Promise((resolve, reject) => {
        const listener = (e2) => {
          const data = e2.data;
          if (
            e2.origin !== window.location.origin ||
        ...
        if (this._frame.parentNode) {
          this._frame.addEventListener(
            ...
            (ev) => {
              ...
              (_a2 = frame.parentNode) == null
                ? void 0
                : _a2.removeChild(frame);
  ...
  var UserManager = class {
    ...
    async _useRefreshToken(state, customRequestArgs = {}) {
